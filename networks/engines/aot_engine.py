...
class AOTEngine(nn.Module):
    ...
    def update_long_term_memory(self, new_long_term_memories):
        ...
        for i, (new_long_term_memory, last_long_term_memory, mca_attn) in enumerate(zip(
                new_long_term_memories, self.long_term_memories,
                self.AOT.long_term_memory_attention)):

            updated_e = []
            for j, (new_e, last_e, mca) in enumerate(zip(new_long_term_memory, last_long_term_memory, mca_attn)):
                if new_e is None or last_e is None:
                    updated_e.append(None)
                else:
                    if last_e.shape[0] == new_e.shape[0]:
                        updated_e.append(torch.cat([new_e, last_e], dim=0))
                    else:
                        updated_e.append(torch.cat([mca(new_e, last_e[:new_e.shape[0], :, :], self.enc_size_2d[0], self.enc_size_2d[1]), last_e[new_e.shape[0]:, :, :]], dim=0))
