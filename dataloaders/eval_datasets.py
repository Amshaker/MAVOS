...
class YOUTUBEVOS_Test(object):
    def __init__(self,
        ...
        print("SEQ LIST FILE IS: ", self.seq_list_file)
        self._check_preprocess()
        print("self.ann_f is ", self.ann_f)
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.isfile(
            ...
            try:
                ...
                print(inst)
                print('Failed to create a result folder for sequence {}.'.format(seq_name))
            source_label_path = os.path.join(self.label_root, seq_name,
                                             labels[0])
            result_label_path = os.path.join(self.result_root, seq_name,
                                             labels[0])
...
class LVOS_Test(object):
    def __init__(
        ...
        self.db_root_dir = root
        self.image_root = os.path.join(root, split+'_480p', "JPEGImages")
        #if split == "val":
        self.label_root = os.path.join(root, split+'_480p', "Annotations_convert")
        if split =='test':
            self.label_root = os.path.join(root, split+'_480p', "Annotations")
        self.result_root = result_root
        self.rgb = rgb
        self.transform = transform
        self.seqs = sorted(os.listdir(self.image_root))
    ...
    def __getitem__(self, idx):
        ...
        images = []
        labels = []
        image_root = os.path.join(self.image_root, seq_name)
        label_root = os.path.join(self.label_root, seq_name)
        _images = list(np.sort(os.listdir(image_root)))
        labels = list(np.sort(os.listdir(label_root)))

        start_label = labels[0].split(".")[0]
        images = []
        for _im in _images:
            _iim = _im.split(".")[0]
            if int(_iim) >= int(start_label):
                images.append(_im)
        ...
        try:
            if not os.path.isfile(os.path.join(self.result_root, seq_name, labels[0])):
                if not os.path.exists(os.path.join(self.result_root, seq_name)):
                    os.makedirs(os.path.join(self.result_root, seq_name))
                shutil.copy(
                    os.path.join(label_root, labels[0]),
                    os.path.join(self.result_root, seq_name, labels[0]),
                )
        except Exception as inst:
            print(inst)
...
class LVOS_Test__(object):
    ...
    def __getitem__(self, idx):
        ...
        labels = []
        for obj_n in obj_names:
            folder_dir = self.image_root + '/' + seq_name
            start = data[obj_n]["frame_range"]["start"]
            end = data[obj_n]["frame_range"]["end"]
            #print("Start i ", start)
            for image in os.listdir(folder_dir):
                #print ("Image is ", image)
                img_id = int(image.split('.')[0])
                if img_id == int(start):
                #if img_id >= int(start) and img_id <= int(end):
                    #print("Image name is ", image)
            #images += map(lambda x: x + '.jpg', list(data[obj_n]["frames"]))
                    images.append(image.split('.')[0] + '.jpg')
                    labels.append(image.split('.')[0] + '.png')
        images = np.sort(np.unique(images))
        labels = np.sort(np.unique(labels))
